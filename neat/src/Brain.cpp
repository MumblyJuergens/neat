...
[[nodiscard]] real_t Brain::difference(const Brain &representative, const Config &cfg) const noexcept
{
    ...
    std::ranges::set_intersection(tSynapses, rSynapses, std::back_inserter(tMatching), {}, &Synapse::innovation,
                                  &Synapse::innovation);
    std::ranges::set_intersection(rSynapses, tSynapses, std::back_inserter(rMatching), {}, &Synapse::innovation,
                                  &Synapse::innovation);
    assert(tMatching.size() == rMatching.size());
    real_t weightDifference{};
    // TODO: Use zip?
    mj::loop(tMatching.size(), [&](const std::size_t i) {
        weightDifference += mj::difference(tMatching[i].weight(), rMatching[i].weight());
    });
...
[[nodiscard]] Brain Brain::crossover(const Brain &best, const Brain &worst, const Config &cfg, Random &random)
{
    ...
    for (const auto &synapse : best.m_synapses) {
        ...
        const auto matchingSynapse = std::ranges::find_if(
            worst.m_synapses, mj::magic_callable(&Synapse::innovation, std::equal_to{}, synapse.innovation()));
    ...
    for (int layer{}; layer < m_layer_count - 1; ++layer) {
        for (const auto &neuronIn : m_neurons | mj::filter(&Neuron::layer, std::equal_to{}, layer)) {
            for (const auto &neuronOut : m_neurons | mj::filter(&Neuron::layer, std::greater{}, layer)) {
                if (std::ranges::any_of(m_synapses, [&](const Synapse &s) {
                        return s.in() == neuronIn.number() && s.out() == neuronOut.number();
                    }))
                    continue;
...
[[nodiscard]] std::vector<real_t> Brain::run_network(const std::vector<real_t> &inputs, activator_f *activator) noexcept
{
    ...
    for (int i{1}; i < m_layer_count; ++i) {
        for (auto &neuron : m_neurons | mj::filter(&Neuron::layer, std::equal_to{}, i)) {
            real_t value = 0;
            for (const auto &synapse : m_synapses | mj::filter(&Synapse::out, std::equal_to{}, neuron.number()) |
                                           std::views::filter(&Synapse::enabled))
            {
...
std::string Brain::chart() const noexcept
{
    ...
    for (const auto &synapse : m_synapses | std::views::filter(&Synapse::enabled)) {
        std::println(stream, "{}({}) -- {} --> {}({})", synapse.in(), m_neurons.at(mj::sz_t(synapse.in())).layer(),
                     synapse.weight(), synapse.out(), m_neurons.at(mj::sz_t(synapse.out())).layer());
    }
    return stream.str();
}
