...
class [[nodiscard]] Random final
{
  ...
  public:
    ...
    [[nodiscard]] static real_t canonical_skewed_high(const real_t strength)
    {
        return 1.0_r - std::pow(1.0_r - canonical(), strength);
    }

    [[nodiscard]] static real_t canonical_skewed_low(const real_t strength)
    {
        return 1.0_r - std::pow(canonical(), strength);
    }
};
